...
class TestArtifactAutoLoadingLLMNamespace:
    """Test auto-loading from LLMNamespace (Python execution)."""

    def test_artifact_auto_loads_from_python_namespace(self, agent, artifact):
        """Test that accessing artifact from Python namespace auto-loads it."""
        # Store artifact in variables
        agent.state.variables["$test_artifact"] = artifact
        ...
        # Create PythonExecutor and LLMNamespace
        executor = PythonExecutor(agent)
        namespace = LLMNamespace(executor)

        # Access the artifact via namespace
        result = namespace["test_artifact"]
    ...
    def test_artifact_not_duplicated_from_python_namespace(self, agent, artifact):
        """Test that accessing artifact from Python doesn't duplicate when loaded."""
        # Store artifact in variables
        agent.state.variables["$test_artifact"] = artifact
...
class TestCallStackIsArtifactLoaded:
    ...
    def test_artifact_name_normalized_with_dollar_prefix(self, agent, artifact):
        """Test that artifact name is normalized with $ prefix."""
        artifact_msg = ArtifactLLMMessage(artifact)
        agent.state.call_stack.add_llm_message(artifact_msg)
