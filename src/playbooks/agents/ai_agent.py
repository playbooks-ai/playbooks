...
class AIAgent(BaseAgent, ABC, metaclass=AIAgentMeta):
    ...
    def create_begin_playbook(self):
        ...
        code_block = f"""# Auto-generated by Playbooks AI
#
# Multiple playbooks in agent "{self.klass}" have a BGN trigger, i.e.
# multiple playbooks need to be called when the program starts.
# This auto-generated playbook is used to call all of them sequentially.
...
"""
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", prefix=prefix, suffix=".py", delete=False
        ) as f:
            f.write(code_block)
            f.flush()
            file_path = f.name

        ast = markdown_to_ast(code_block, source_file_path=file_path)
        refresh_markdown_attributes(ast)

        debug("BGN Playbook Code Block: " + ast["markdown"])
        new_playbook = PythonPlaybook.create_playbooks_from_code_block(
            ast["markdown"],
            self.namespace_manager,
            ast.get("source_file_path"),
            ast.get("line_number"),
        )
